...
/transcript       # Process call transcripts into marketing intelligence
/report             # Website performance report (GA/GSC)
/report-setup       # Configure website report data sources
```
...
# Command Center (Interactive Agent UI)
cd command-center && bun run start          # http://localhost:3457
...
| **Command Center** | Interactive browser UI for agents | http://localhost:3457 |
...
content/                        # Generated content ready for publishing
  └── transcripts/              # Transcript extraction logs and voice notes
docs/                           # Reference documentation
  └── intelligence/             # Social media intelligence data
      └── internal/             # Auto-maintained intelligence files (write-back targets)
...
config/                         # Centralized configuration
  ├── brand.json                # Colors, style directives, pillars
  ├── partners.json             # Partner accounts and names
...
.claude/                        # Agent configurations & hooks
  ...
  └── skills/
      ├── images/               # /images command - AI image generation toolkit
      ├── social-intel/         # /intel command - Social media intelligence
      └── transcript/           # /transcript command - Call transcript extraction
...
website-report/                 # Website performance reports (GA/GSC via /report)
command-center/                 # Interactive browser UI for agent operations
  ├── server.ts                 # Bun HTTP + WebSocket server (port 3457)
  ├── sessions.ts               # SDK session manager (streaming, permissions)
  ├── protocol.ts               # WebSocket message type definitions
  ├── agents.ts                 # Agent metadata loader
  ├── intel.ts                  # Intel freshness checker
  └── ui/index.html             # Complete UI (single-file, no build)
factory-visualizer-portable/    # Factorio-style agent activity visualizer
```

---

# 🏭 **FACTORY FLOOR VISUALIZER**
...
# 🖥️ **COMMAND CENTER** *(Optional — Proof of Concept)*

Optional browser interface for operating your AI marketing organization visually. All CLI commands (`/cmo`, `/analyst`, etc.) work fully without this. The Command Center provides the same agent capabilities in a browser window — streaming chat, tool visibility, permission handling, and multi-session support.
...
- **Agent Sidebar**: All 17 agents with keyboard hotkeys, color-coded by role
- **Streaming Chat**: Token-by-token markdown rendering with syntax highlighting
- **Tool Cards**: Collapsible display of every tool call (Read, Write, Bash, Glob, etc.)
- **Permission Handling**: In-browser Allow/Deny for dangerous operations
- **Multi-Session Tabs**: Run multiple agents simultaneously, switch between them
- **Context Panel**: Brand status, intel freshness, context file existence, live stats

## Architecture

```
Browser (HTML/CSS/JS)  ←—WebSocket—→  Bun Server  ←—SDK—→  Claude Code CLI
     :3457                              sessions.ts          (your auth)
```

Powered by `@anthropic-ai/claude-agent-sdk`. The SDK spawns Claude Code CLI as a subprocess, using your existing claude.ai authentication.

## Relationship to CLI

The Command Center is an **alternative interface** to CLI commands like `/cmo` and `/analyst`. Same agents, same capabilities — visual feedback instead of terminal output.

See `command-center/README.md` for full documentation.
...
- `/report` to generate website performance reports
- `/transcript` to extract intelligence from call transcripts
...
| Session ledger (auto) | `docs/intelligence/internal/context-intelligence-ledger.md` |
| Competitive intelligence (auto) | `docs/intelligence/internal/competitive-intelligence-tracking.md` |
| Performance history (auto) | `docs/intelligence/internal/performance-analysis-history.md` |
| Seasonal patterns (auto) | `docs/intelligence/internal/seasonal-patterns.md` |
| Website performance reports | `website-report/` (use `/report`) |
...
| Command Center | `command-center/` (http://localhost:3457) |
| Command Center SaaS notes | `docs/strategic-planning/command-center-saas-notes.md` |
...
- **Command Center**: `command-center/README.md` (interactive agent UI, POC)
- **SaaS Strategy**: `docs/strategic-planning/command-center-saas-notes.md`
...
- **SessionStart** — Detects missing `client-context/brand/` and prompts `/onboard` on first run
- **SessionEnd** — Appends session metadata to `docs/intelligence/internal/context-intelligence-ledger.md`
