#!/usr/bin/env python3
"""
Quick validation script for skills - validates basic structure and frontmatter
...
def validate_skill(skill_path):
    ...
    ALLOWED_PROPERTIES = {'name', 'description', 'license', 'allowed-tools', 'metadata', 'tags', 'archetype'}
    ...
    if tags:
        ...
        for tag in tags:
            if not isinstance(tag, str):
                return False, f"Each tag must be a string, got {type(tag).__name__}"
            if not re.match(r'^[a-z0-9-]+$', tag):
                return False, f"Tag '{tag}' should be lowercase hyphen-case (lowercase, digits, hyphens only)"
            if len(tag) > 32:
                return False, f"Tag '{tag}' too long ({len(tag)} chars). Maximum: 32"

    # Validate archetype (optional)
    archetype = frontmatter.get('archetype')
    if archetype:
        valid_archetypes = {'guidance', 'toolkit', 'router', 'document', 'meta'}
        if archetype not in valid_archetypes:
            return False, f"Invalid archetype '{archetype}'. Must be one of: {', '.join(sorted(valid_archetypes))}"
